# Biblioteca Familiar

Aplicación privada para organizar los libros de una familia desde Windows y
Android. Cada familia crea su propia biblioteca independiente y puede unir sus
dispositivos mediante un código o un QR.

## Descarga sencilla

- [Pack completo para Windows y Android](https://github.com/mgc2025gpt-spec/biblioteca-familiar-descargas/releases/download/v0.5.0/Biblioteca-Familiar-Pack-Completo.zip)
- [Instalador para Windows 11](https://github.com/mgc2025gpt-spec/biblioteca-familiar-descargas/releases/download/v0.5.0/Biblioteca-Familiar-Windows-Instalador.exe)
- [Aplicación para Android](https://github.com/mgc2025gpt-spec/biblioteca-familiar-descargas/releases/download/v0.5.0/Biblioteca-Familiar-Android.apk)
- [Código QR para instalar en Android](https://github.com/mgc2025gpt-spec/biblioteca-familiar-descargas/releases/download/v0.5.0/QR-Instalar-Android.png)

La opción recomendada es descargar el **pack completo**, abrir el ZIP y leer
`LEEME_PRIMERO.html`. La aplicación permite fotografiar varios libros, revisar
las fichas detectadas, buscar, organizar ubicaciones y exportar listas a Excel o
Word.

## Sincronización familiar

En el primer dispositivo, abre **Ajustes > Sincronización familiar > Crear
biblioteca familiar**. En los demás dispositivos usa **Unirse a una biblioteca**
y escribe el mismo código o escanea el QR. No compartas ese código fuera de la
familia.

## Actualización desde Android 0.4.2

Antes de cambiar: sincroniza, anota el código familiar y desinstala la versión
antigua una sola vez. Después instala la 0.5.0 y vuelve a unirte con el código.
Las próximas versiones firmadas podrán instalarse como actualización normal.

Este repositorio contiene únicamente archivos instalables públicos. No contiene
bibliotecas familiares, fotografías, contraseñas, claves privadas ni el código
fuente del programa.
